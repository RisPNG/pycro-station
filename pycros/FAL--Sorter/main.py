...
# The following implements a 1-to-1 functional port of the provided
# Excel VBA macro into Python using openpyxl within the Pycro UI.
...
def _clean_text(input_text: str) -> str:
    if input_text is None:
        return ''
    s = str(input_text)
    # remove LF, CR; tabs to space; nbsp to space
    s = s.replace('\n', '').replace('\r', '')
    s = s.replace('\t', ' ')
    s = s.replace('\xa0', ' ')
    s = s.strip()
...
def _is_serial_row(row_vals: List[Any], last_col: int) -> bool:
    # Column A contains "Serial Number" and all others empty
    a = row_vals[0]
    if a is None:
        return False
...
def _auto_fit_columns(ws: Worksheet):
    # basic auto-fit by max text width; approximate
    dims: Dict[int, int] = {}
...
def _delete_columns(ws: Worksheet):
    # Delete higher-indexed first, following VBA order: R, O, J, I, H, G, E
    for col_letter in ['R', 'O', 'J', 'I', 'H', 'G', 'E']:
        try:
            idx = column_index_from_string(col_letter)
            if idx <= (ws.max_column or 0):
                ws.delete_cols(idx, 1)
        except Exception:
            # mirror VBA's On Error Resume Next
            pass


def _format_column_L(ws: Worksheet):
    # Left align and set number format "0" to avoid scientific notation
    max_row = ws.max_row or 1
...
def _process_sheet(source_data: List[List[Any]], target_ws: Worksheet, last_row: int, last_col: int, sheet_type: str):
    ...
    # Prepare filtered buffer (1-based in VBA; here 0-based Python)
    ...
    filtered_count = 1  # next write index (0-based)

    # Filter rows based on Column E first two chars
    for i in range(1, last_row):  # skip header (0)
        included = False
        ...
        if col_e not in (None, ''):
            ...
            # If ALL and Column E empty, we do not include the row itself here
            pass
        ...
        if included:
            ...
            if i < len(source_data):
                ...
                filtered_count += 1
            # also copy a possible serial row beneath
    ...
    # Write categorized data or just cleaned header
    if filtered_count > 1:
        _categorize_and_write_data(target_ws, filtered, filtered_count, last_col)
    else:
        # only write cleaned header
        for j in range(last_col):
            hv = filtered[0][j]
...
def _categorize_and_write_data(ws: Worksheet, data_array: List[List[Any]], data_rows: int, last_col: int):
    ...
    categories: List[Tuple[str, str]] = [
        ...
        ('MV', 'Motor Vehicle'),
        ('UC', 'Unknown Category'),  # fallback
    ]
    known_count = 11  # last is UC

    # collections of row indices per category (store indices into data_array)
    category_data: List[List[int]] = [[] for _ in range(12)]

    # serial number map keyed by data row index
    ...
    for i in range(1, data_rows):
        ...
        # determine category from original Column D (index 3), chars 3-4
        category_index = 0
    ...
    # Writing stage
    out_serial_col = last_col + 2  # Description inserted at B, plus Serial Number at end

    current_row = 1
    # header
    ws.cell(row=current_row, column=1, value='Asset ID')
    ws.cell(row=current_row, column=2, value='Description')
    for j in range(1, last_col):  # original B..lastCol => output C..
    ...
    # Totals accumulators for original numeric columns J..Q (10..17), excluding 10 (Life)
    grand_totals: Dict[int, float] = {k: 0.0 for k in range(10, 18)}
    sum_label_col = 10  # J after Description
    sum_start_out_col = 11  # K
    sum_end_out_col = min(last_col + 1, 18)  # cap at R (18), exclude Serial Number
    ...
    for i, cat_rows in enumerate(category_data, start=1):
        ...
        # Bold and top border for subtotal range
        from openpyxl.styles import Border, Side
        rng_start = ws.cell(row=current_row, column=sum_label_col)
        rng_end_col = sum_end_out_col
        rng_end = ws.cell(row=current_row, column=rng_end_col)
        top = Border(top=Side(style='thin'))
        # apply to range
        for c in ws.iter_rows(min_row=current_row, max_row=current_row, min_col=sum_label_col, max_col=rng_end_col):
            for cell in c:
                cell.font = Font(bold=True) if cell.font is None else Font(name=cell.font.name, size=cell.font.size, bold=True)
                # Assign top border
    ...
    for c in ws.iter_rows(min_row=current_row, max_row=current_row, min_col=sum_label_col, max_col=sum_end_out_col):
        for cell in c:
            # double top and bottom border per VBA intent
            cell.font = Font(bold=True) if cell.font is None else Font(name=cell.font.name, size=cell.font.size, bold=True)
            cell.border = Border(top=Side(style='double'), bottom=Side(style='thin'))

    # Ensure 2 decimal places with comma separators on numeric band for all rows
    ...
    if sum_start_out_col <= sum_end_out_col:
        for r in range(2, last_out_row + 1):
            for c in range(sum_start_out_col, sum_end_out_col + 1):
                ...
                # only set if numeric value exists
                if isinstance(cell.value, (int, float)):
                    cell.number_format = '#,##0.00'


def _copy_after(wb, ws_to_move: Worksheet, after_ws: Worksheet):
    # Move ws_to_move to be placed right after after_ws
    try:
        ...
        # best-effort; order is not critical for correctness
        pass
...
def _prepare_target_sheets(wb, source_ws: Worksheet) -> Tuple[Worksheet, Worksheet, Worksheet]:
    # Create three new sheets by copying the source sheet, placed after it sequentially
...
def _process_master(wb):
    # VBA MasterStart equivalent for a single open workbook
...
def _xlrd_cell_value(book, cell):
    ...
    # numbers, text, empty, error -> pass through
    return cell.value
...
def _success_msg() -> str:
    return 'Processing complete.'


def _summary_msg(wb) -> str:
    pr = 'yes' if _has_processed_sheets(wb) else 'no'
    return f'Post status: PRAI/VIETNAM/ALL present={pr}'
...
def _process_and_format(wb):
    _process_master(wb)
    prai, vn, all_ws = _get_sheet(wb, 'PRAI'), _get_sheet(wb, 'VIETNAM'), _get_sheet(wb, 'ALL')
    _format_all_three(prai, vn, all_ws)
...
def _safe_process(path: str, log):
    # Backward-compatible simple boolean return used by some helpers
    ok, _ = _safe_process_with_output(path, log)
    return ok
